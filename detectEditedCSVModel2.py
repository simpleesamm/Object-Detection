...
reclassification_arr = []
with open('presubmission.csv','w',encoding='UTF8',newline='') as f:
    ...
    for count, image_path in enumerate(images, 1):
        ...
        if (predicted_class == "Black-grass") or (predicted_class == "Loose Silky-bent" ):
            reclassification_arr.append((image_path,predicted_class,count+1)) #for comparison purposes #count+1 because first row is header row
...
presubmission_arr=[]
with open('presubmission.csv') as presub:
    r = csv.reader(presub)
    presubmission_arr = list(r)

    for image_tuple in reclassification_arr:
        image_path = image_tuple[0]
        original_classification = image_tuple[1]
        image_row = image_tuple[2]
        print(image_path)
        print(original_classification)
        print(image_row)

        ####################################################### do second classification using image_path#########################################


        ##########################################################################################################################################
        #presubmission_arr[image_row][1] = #put new predicted class here

# with open('submission.csv','w',encoding='UTF8',newline='') as f:
#     writer = csv.writer(f)
#     writer.writerows(presubmission_arr)
